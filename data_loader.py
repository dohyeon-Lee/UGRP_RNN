...
class data_loader():
    ...
    def __init__(self, train_datasize, test_datasize, sequence_length, batch_size, device):
        self.device = device
        ...
        for i in range(0,self.train_datasize):
            ...
            if i == 0:
                ...
                train_input_seq, train_output_seq = self.seq_data(self.train_input[i], self.train_output[i], sequence_length)
                ...
                self.train_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=False)
                ...
                train = torch.utils.data.TensorDataset(self.train_input_seq[i], self.train_output_seq[i])
                self.train_loader.append(torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=False))
        #make test dataset
        ...
        for i in range(0,self.test_datasize):
            ...
            if i == 0:
                ...
                test = torch.utils.data.TensorDataset(self.test_input_seq[i], self.test_output_seq[i])
                self.test_loader.append(torch.utils.data.DataLoader(test, batch_size=batch_size, shuffle=False))
